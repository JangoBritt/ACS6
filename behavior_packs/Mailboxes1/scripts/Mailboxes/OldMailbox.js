...
export class MailBoxes {
    ...
    getMailboxByCoordinates(coordinates) {
		const allMailboxes = this.getAllMailboxes();
		return allMailboxes.find(mailbox =>
			mailbox.coordinates.x === coordinates.x &&
			mailbox.coordinates.y === coordinates.y &&
			mailbox.coordinates.z === coordinates.z
		);
	}
}